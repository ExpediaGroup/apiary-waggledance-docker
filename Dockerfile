...
RUN yum -y update && \
  yum install -y java-${JAVA_VERSION}-openjdk \
    ...
    util-linux \
    jq \
    https://oss.sonatype.org/content/repositories/snapshots/com/expediagroup/waggle-dance-rpm/${WAGGLE_DANCE_VERSION}/waggle-dance-rpm-4.1.1-20251015.132842-2.rpm \
  && yum clean all \
  && rm -rf /var/cache/yum
